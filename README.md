...
Horizon is a simple weather app built completely with Jetpack Compose. Under the hood, it uses the [Open-Meteo Weather API](https://open-meteo.com) to fetch the data. It also uses [Google's Gemini client SDK](https://ai.google.dev) to display a short, whimsical summary of the current weather of a particular location. This app uses the Material 3 design system and also supports dynamic colors on supported devices.
...
- [Gemini](https://ai.google.dev) for generating a short, whimsical summary of the current weather of a particular location.
